...
class LogRepository(Protocol):
    ...
    def create_preview(
        self, *, actor: str, now: datetime | None = None, ttl_seconds: int = 300
    ) -> DeletionPreview: ...
    def delete_preview(
...
class MemoryRepository:
    ...
    def append(self, event: LogEvent) -> dict[str, Any]:
        ...
        row = _row(
            event,
            len(self._events) + 1,
            self._events[-1]["integrity_digest"] if self._events else "",
        )
    ...
    def get(self, event_id: UUID | str) -> dict[str, Any] | None:
        value = str(event_id)
        return next(
            (dict(row) for row in self._events if row["event_id"] == value), None
        )
    ...
    def set_policy(self, policy: RetentionPolicy, *, actor: str) -> RetentionPolicy:
        ...
        self._journal(
            ...
            actor,
            {"enabled": policy.enabled, "max_entries": policy.max_entries},
        )
        return policy
    ...
    def create_preview(
        ...
        if not policy.enabled:
            ...
            ids = list(
                dict.fromkeys(
                    ...
                    if UUID(row["event_id"]) not in held
                )
            )
        ...
        preview = DeletionPreview(
            ...
            self.object_uid,
            tuple(ids),
            hashlib.sha256(selection.encode()).hexdigest(),
            actor,
            now,
    ...
    def delete_preview(
        ...
        if (
            not preview
            or preview.is_expired()
            or preview.actor != actor
            or not reason.strip()
        ):
            raise PreviewExpired(
                "deletion preview is missing, expired, or not owned by the actor"
            )
        held = {
            hold.event_id for hold in self.list_holds(active_only=True) if hold.event_id
        }
        ...
        self._events = [
            row for row in self._events if UUID(row["event_id"]) not in selected
        ]
    ...
    def create_hold(self, hold: LegalHold) -> LegalHold:
        ...
        self._journal(
            ...
            hold.actor,
            {
                ...
                "event_id": str(hold.event_id) if hold.event_id else None,
            },
        )
        return hold
    ...
    def release_hold(self, hold_id: UUID | str, *, actor: str) -> LegalHold:
        ...
        self._journal(
            "governance.hold.released", actor, {"hold_id": str(released.hold_id)}
        )
        return released
    ...
    def verify(self) -> dict[str, Any]:
        ...
        for row in self._events:
            expected = digest(row)
            if (
                row.get("previous_digest") != previous
                or row.get("integrity_digest") != expected
            ):
                return {
                    "ok": False,
...
class ZODBRepository(MemoryRepository):
    ...
    def _load(self) -> None:
        ...
        self._holds = {
            UUID(str(key)): LegalHold(**value)
            for key, value in store.get("holds", {}).items()
        }
        self._policy = RetentionPolicy(**store.get("policy", {}))
        self._previews = {
            UUID(str(key)): DeletionPreview(
                UUID(str(value["preview_id"])),
                value["object_uid"],
    ...
    def _save(self) -> None:
        ...
        store["policy"] = {
            ...
            "older_than_days": self._policy.older_than_days,
            "max_entries": self._policy.max_entries,
            "policy_id": self._policy.policy_id,
            "legal_basis": self._policy.legal_basis,
        }
        store["holds"] = {
            str(key): {
            ...
            for key, value in self._holds.items()
        }
        store["previews"] = {
            str(key): {
    ...
    def set_policy(self, policy: RetentionPolicy, *, actor: str) -> RetentionPolicy:
        result = super().set_policy(policy, actor=actor)
        self._save()
        return result

    def create_hold(self, hold: LegalHold) -> LegalHold:
        result = super().create_hold(hold)
        self._save()
        return result
    ...
    def release_hold(self, hold_id: UUID | str, *, actor: str) -> LegalHold:
        result = super().release_hold(hold_id, actor=actor)
        self._save()
        return result
