...
def _bounded_text(
    name: str, value: Any, maximum: int, *, required: bool = False
) -> str:
    if (
...
class LogEvent:
    ...
    def __post_init__(self) -> None:
        ...
        try:
            severity = Severity(
                "warning"
                if str(self.severity).casefold() == "warn"
                else str(self.severity).casefold()
            )
        ...
        if self.info_url is not None:
            ...
            if (
                ...
                raise ValidationError(
                    "info_url must be an https or configured relative URL"
                )
...
class RetentionPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or not 0 < self.max_entries <= 1_000_000
        ):
...
class LegalHold:
    ...
    def __post_init__(self) -> None:
        ...
        if self.released_at is not None:
            object.__setattr__(
                self, "released_at", utc(self.released_at, "released_at")
            )
