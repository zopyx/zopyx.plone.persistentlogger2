...
class SQLiteRepository(MemoryRepository):
    ...
    def _create_schema(self) -> None:
        ...
        """)
        if self.connection.execute("SELECT COUNT(*) FROM audit_schema_version").fetchone()[0] == 0:
    ...
    def health(self) -> dict[str, Any]:
        result = super().health()
        result.update(schema_version=self.connection.execute("SELECT MAX(version) FROM audit_schema_version").fetchone()[0], transaction_mode=self.transaction_mode)
        return result
