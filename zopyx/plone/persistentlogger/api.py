...
def log_event(context: Any, comment: str, *, event_type: str = "application",
              severity: str = "info", actor: str | None = None,
              target: str | None = None, info_url: str | None = None,
              details: Any = None, occurred_at: datetime | None = None,
              created_at: datetime | None = None) -> dict[str, Any]:
    event = LogEvent(comment=comment, object_uid=object_uid(context), event_type=event_type,
                     severity=severity, actor=actor or _current_actor(), target=target,
                     info_url=info_url, details=details, occurred_at=occurred_at,
...
def set_retention_policy(context: Any, policy: RetentionPolicy, *, actor: str) -> RetentionPolicy:
    return repository_for(context).set_policy(policy, actor=actor)


def export_events(context: Any, *, format: str = "json", max_rows: int = 10_000,
                  max_bytes: int = 5 * 1024 * 1024, **filters: Any) -> tuple[bytes, str, str]:
    ...
    if format == "json":
        ...
        fields = ["event_id", "object_uid", "created_at", "occurred_at", "actor", "event_type", "severity", "target", "comment", "info_url", "details", "schema_version", "sequence", "previous_digest", "integrity_digest"]
        writer = csv.DictWriter(stream, fieldnames=fields, extrasaction="ignore")
        writer.writeheader()
        for row in rows:
            safe = dict(row)
            safe["details"] = json.dumps(safe.get("details"), ensure_ascii=False, sort_keys=True)
...
def clear(*args: Any, **kwargs: Any) -> None:
    raise ValidationError("clear is removed; use retention preview and governed deletion")
