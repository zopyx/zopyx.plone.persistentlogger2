...
def normalize(
    ...
    *,
    redact: bool = True,
    sensitive_keys: Iterable[str] = DEFAULT_SENSITIVE,
) -> Any:
    ...
    if isinstance(value, Mapping):
        ...
        for key, item in value.items():
            ...
            result[key] = (
                REDACTED
                if redact and _key_matches(key, sensitive_keys)
                else normalize(item, redact=redact, sensitive_keys=sensitive_keys)
            )
        return result
    if isinstance(value, Sequence) and not isinstance(value, (bytes, bytearray)):
        return [
            normalize(item, redact=redact, sensitive_keys=sensitive_keys)
            for item in value
        ]
